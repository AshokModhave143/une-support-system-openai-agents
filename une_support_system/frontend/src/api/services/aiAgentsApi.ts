...
import { ChatPostMessageRequest } from '../types';
...
export const AiAgentsApiService = {
    post: async (message: string) => {
        try {
            const payload: ChatPostMessageRequest = { message }
            const response = await axios.post(`${BASE_URL}${API_ENDPOINT}`, payload, { method: 'POST', headers: { 'Content-Type': 'application/json' }});
            ...
            return { status: response.status, error: null, data: agentMessage };
        } catch (error) {
            console.error('Error in sending message to AI agent:', error);
            return { status: 500, error: `Error in sending message to AI agent: ${error}`, data: null };
        }
    }
};
